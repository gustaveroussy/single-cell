...
def grp_add_bcr_params_bcr_files(wildcards):
    return ",".join([ os.path.normpath(bcrfile) for bcrfile in list(dict.fromkeys(dic_GRP_ADD_BCR_INFO[wildcards.grp_add_bcr_output]['GRP_ADD_BCR_INPUT_CSV_BCR'].split(","))) ])
...
def grp_add_bcr_params_output_folder(wildcards):
    return os.path.dirname(wildcards.grp_add_bcr_output) + "/"
...
rule grp_add_bcr_ge:
    ...
    log:
        "logs/grp_add_bcr_ge{grp_add_bcr_output}.log"
    benchmark:
        "benchmark/grp_add_bcr_ge{grp_add_bcr_output}.tsv"
    threads:
        1
    resources:
