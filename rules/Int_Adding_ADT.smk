...
def int_add_adt_params_input_folder(wildcards):
    return ",".join([ os.path.normpath(kallisto_folder + "/") for kallisto_folder in list(dict.fromkeys(dic_INT_ADD_ADT_INFO[wildcards.int_add_adt_output]['INT_ADD_ADT_INPUT_DIR_ADT'].split(","))) ])
...
def int_add_adt_params_output_folder(wildcards):
    return os.path.dirname(wildcards.int_add_adt_output) + "/"
...
rule int_add_adt_ge:
    ...
    log:
        "logs/int_add_adt_ge{int_add_adt_output}.log"
    benchmark:
        "benchmark/int_add_adt_ge{int_add_adt_output}.tsv"
    threads:
        1
    resources:
    ...
    shell:
        ...
        --gene.names  {INT_ADD_ADT_GENE_NAMES} &> {log}
        """
