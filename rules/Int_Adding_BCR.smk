...
rule int_add_bcr_ge:
    ...
    threads:
        1
    resources:
        mem_mb = lambda wildcards, attempt: INT_ADD_BCR_MEM if (INT_ADD_BCR_MEM is not None) else min(5120 + attempt * 3072, 20480),
        time_min = lambda wildcards, attempt: INT_ADD_BCR_TIME if (INT_ADD_BCR_TIME is not None) else min(attempt * 120, 200)
    shell:
        """
