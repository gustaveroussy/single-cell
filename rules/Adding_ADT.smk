...
def add_adt_input(wildcards):
    ...
    if "Alignment_countTable_ADT" in STEPS:
        files = [os.path.normpath(kallisto_folder + "/Materials_and_Methods.txt")]
    else:
...
def add_adt_params_sing(wildcards):
    ...
    concat = " -B " + PIPELINE_FOLDER + "," + rda_folder + "," + kallisto_folder
    return concat
...
def add_adt_params_input_folder(wildcards):
    return os.path.normpath(dic_ADD_ADT_INFO[wildcards.add_adt_output]['ADD_ADT_INPUT_DIR_ADT']) + "/"
...
def add_adt_params_output_folder(wildcards):
    return os.path.normpath(os.path.dirname(wildcards.add_adt_output)) + "/"
...
rule add_adt_ge:
    ...
    log:
        "logs/add_adt_ge{add_adt_output}.log"
    benchmark:
        "benchmark/add_adt_ge{add_adt_output}.log"
    threads:
        1
    resources:
    ...
    shell:
        ...
        {SINGULARITY_ENV} \
        Rscript {PIPELINE_FOLDER}/scripts/pipeline_ADT.R \
        ...
        --gene.names  {ADD_ADT_GENE_NAMES} &> {log}
        """
