...
def int_add_tcr_params_tcr_files(wildcards):
    return ",".join([ os.path.normpath(tcrfile) for tcrfile in list(dict.fromkeys(dic_INT_ADD_TCR_INFO[wildcards.int_add_tcr_output]['INT_ADD_TCR_INPUT_CSV_TCR'].split(","))) ])
...
def int_add_tcr_params_output_folder(wildcards):
    return os.path.dirname(wildcards.int_add_tcr_output) + "/"
...
rule int_add_tcr_ge:
    ...
    log:
        "logs/int_add_tcr_ge{int_add_tcr_output}.log"
    benchmark:
        "benchmark/int_add_tcr_ge{int_add_tcr_output}.tsv"
    threads:
        1
    resources:
