...
if (is.null(individual.norm)) individual.norm <- FALSE
...
print(paste0("individual.norm: ",individual.norm))
...
## Merge data
cat("\nMerge data...\n")
sobj <- merge(x = sobj.list[[1]], y = sobj.list[-1], add.cell.ids = names(sobj.list), project = name.grp, merge.data = FALSE) #merge object
sobj[["RNA"]] <- SeuratObject::JoinLayers(sobj[["RNA"]]) # if we make the individual normalization JoinLayers or not doesn't change the final result
if(individual.norm){
  sobj[["RNA"]] <- split(sobj[["RNA"]], f = sobj@meta.data[["orig.ident"]]) #split layers of RNA assay -> redefinition of layers necessary for individual normalizations
}
...
sobj@misc$parameters$Materials_and_Methods$Grouped_analysis_Norm_DimRed_Eval <- paste0("Seurat (version ",sobj@misc$technical_info$Seurat,") was applied for further data processing. Datasets were merged with the merge() function",
        if(individual.norm){
          paste0(" and each dataset was normalized independently after spliting layers by samples.")
        }else{
          " and layers was joined."
        },
...
cat("\nSaving object...\n")
sobj@misc$params$analysis_type <- paste0("Grouped analysis; individual normalization: ", individual.norm)
sobj@misc$params$sobj_creation$Rsession <- utils::capture.output(devtools::session_info())
sobj@misc$params$species <- species
sobj@misc$params$group$individual.norm <- individual.norm
