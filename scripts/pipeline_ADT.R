...
cat("\nSaving stat...\n")
save_stat(sobj = sobjADT, sample.names = sample.name.adt, title = sample.name.adt, out.dir = input.dir.adt)
...
if (!all(sort(colnames(sobj@assays$RNA)) == sort(colnames(sobjADT@assays$ADT)))){
  ...
  sobjADT@assays$ADT@counts <- cbind(sobjADT@assays$ADT@layers$counts, rep(rep(0, dim(sobjADT@assays$ADT@layers$counts)[1]),length(cells_without_ADT)))
  colnames(sobjADT@assays$ADT@layers$counts) <- c(colnames_ADT, cells_without_ADT)
...
cor.filtered <- feature.cor(sobj = sobj, assay1 = 'RNA', assay2 = 'ADT', assay1.features = gene.names, assay2.features = rownames(sobj@assays$ADT), slot = slot, cor.method = cor.method, zero.filter = TRUE, gene.names = gene.names, min.cutoff = NULL, max.cutoff = NULL)
cor.df <- cbind(cor.df, cor.filtered)
for (i_min in seq(0, 100, 5)){
    for(j_max in seq(5, 100, 5)){
        if (i_min < j_max) {
            cor.quantile <- feature.cor(sobj = sobj, assay1 = 'RNA', assay2 = 'ADT', assay1.features = gene.names, assay2.features = rownames(sobj@assays$ADT), slot = slot, cor.method = cor.method, zero.filter = FALSE, gene.names = gene.names, min.cutoff = rep(i_min,length(gene.names)), max.cutoff = rep(j_max,length(gene.names)))
...
for (k_gene in 1:length(gene.names)){
    ...
    RNA_data_plot <- feature_plots(sobj, assay = assay, features = gene.names[k_gene], slot = slot, reduction = RNA.reduction, min.cutoff = "q0", max.cutoff = "q100")
    ...
    for (i_min in c(0,5,seq(10, 90, 10),95)){
        for(j_max in c(seq(10, 90, 10),95, 100)){
            if (i_min < j_max) {
                #plot expression
                ADT_data_plot[[paste0("q",i_min,"_q",j_max)]] <- feature_plots(sobj, assay = 'ADT', features = rownames(sobj@assays$ADT)[k_gene], slot = slot, reduction = RNA.reduction, min.cutoff = paste0("q",i_min), max.cutoff = paste0("q",j_max))
...
adt.values <- t(as.matrix(Seurat::GetAssayData(sobj, assay = "ADT", layer = "data")))
