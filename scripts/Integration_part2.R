...
print(paste0("Output directory : ", output.dir.int))
...
cat("\nuMAP plot by sample...\n")
blockpix = 600
png(filename = paste0(clust.dir, '/umap_display_modes/', paste(c(name.int, red.name, 'uMAP.png'), collapse = '_')), width = 1000, height = 1000)
...
dev.off()
png(filename = paste0(clust.dir, '/umap_display_modes/', paste(c(name.int, red.name, 'uMAP_white.png'), collapse = '_')), width = 1000, height = 1000)
print(Seurat::DimPlot(object = sobj, reduction = paste(c(red.name, keep.dims, 'umap'), collapse = '_'), order = sample(x = 1:ncol(sobj), size = ncol(sobj), replace = FALSE), group.by = 'orig.ident', pt.size = solo.pt.size) + ggplot2::ggtitle("uMAP for all samples "))
dev.off()
grid.xy <- grid.scalers(length(unique(sobj@meta.data$orig.ident)))
png(filename = paste0(clust.dir, '/umap_display_modes/', paste(c(name.int, red.name, 'split', 'uMAP_white.png'), collapse = '_')), width = grid.xy[1]*blockpix, height = grid.xy[2]*blockpix)
print(Seurat::DimPlot(object = sobj, reduction = paste(c(red.name, keep.dims, 'umap'), collapse = '_'), group.by = ident.name, split.by = 'orig.ident', pt.size = solo.pt.size, ncol = grid.xy[1]) + ggplot2::ggtitle(paste0("uMAP split on samples")))
dev.off()
...
if (!"data" %in% names(sobj@assays$RNA@layers)) sobj <- Seurat::NormalizeData(sobj, normalization.method = 'LogNormalize', assay = "RNA")
